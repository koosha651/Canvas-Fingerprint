...
(function(){
	...
	function randomMixImageData(window, imageData1, imageData2){
		const data1 = imageData1.data;     // arrays containing the pixel data (R, G, B, A values)
		const data2 = imageData2.data;
		const l = data1.length;		//length of the pixel data arrays.
		if (l === data2.length){
			const rng = randomSupply.getPixelRng(l, window, {});
			
			for (let i = 0; i < l; i += 4){		// Calculating Sign: choosing whether it is -1 or 1
				...
				const [deltaR, deltaG, deltaB, deltaA] = rng(
					signR * (data2[i + 0] - data1[i + 0]),				// find the Random Delta Values
